...
import { getOfflineContributions, removeOfflineContribution, saveOfflineContribution } from '../../database/offline-queue';
import { api } from '../../api/client';
import { fetchWalletData } from './walletSlice';


export const syncOfflineData = createAsyncThunk('offlineQueue/sync', async (_, { dispatch, rejectWithValue }) => {
...
export const enqueueContribution = createAsyncThunk('offlineQueue/enqueue', async (data: { groupId: string, amount: number }) => {
...
const offlineQueueSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromQueue: (state, action) => {
      state.pendingContributions = state.pendingContributions.filter(c => c.id !== action.payload);
    }
  },
