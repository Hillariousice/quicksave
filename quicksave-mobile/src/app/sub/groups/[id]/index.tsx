...
export default function GroupDetailScreen() {
  ...
  const { currentGroup, currentTimeline, currentActivity, isDetailLoading } = useAppSelector(
    (state) => state.groups,
  );
  ...
  const handleChangeStatus = () => {
    ...
    Alert.alert('Change Group Status', 'Update the lifecycle of this savings circle:', [
  ...
  const renderStatusIcon = (status: string) => {
    switch (status) {
      case 'paid':
        return <MaterialCommunityIcons name="check-circle" size={16} color="#34C759" />;
      case 'pending':
        return <View style={styles.pendingDot} />;
      case 'failed':
        return <MaterialCommunityIcons name="close-circle" size={16} color="#FF3B30" />;
      default:
        return null;
    }
  };
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
        ...
        >
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => router.push(`/sub/messages/group/${groupId}`)}
            >
      ...
      <Profiler id="GroupDetailScrollView" onRender={onRenderCallback}>
        ...
        >
              ...
              <View key={item.id} style={[styles.rotationItem, { backgroundColor: theme.inputBg }]}>
                ...
                <View style={styles.rotationInfo}>
                  ...
                  {item.status === 'PROCESSING' && (
                    <Text style={[styles.receivingText, { color: theme.primary }]}>
                      {' '}
                      (Receiving)
                    </Text>
                  )}
          ...
          <View style={[styles.activityContainer, { backgroundColor: theme.inputBg }]}>
            {activities.length === 0 ? (
              <Text style={{ color: theme.textSecondary, textAlign: 'center', padding: 10 }}>
                ...
                return (
                  <View key={activity.id} style={styles.timelineRow}>
                    <View style={styles.timelineGraphic}>
                      ...
                      {!isLast && (
                        <View
                          style={[styles.timelineLine, { backgroundColor: theme.inputBorder }]}
                        />
                      )}
                    </View>
      ...
      <View style={[styles.footer, { backgroundColor: theme.background }]}>
        <TouchableOpacity
          ...
          onPress={() => router.push(`/sub/groups/${groupId}/contribute-member`)}
        >
          <Text style={styles.contributeText}>
            Contribute ₦{currentGroup.contributionAmount.toLocaleString()}
          </Text>
